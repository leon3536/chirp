...
| C-46 | **Two named goal horns.** Two three-part horns ship embedded (C-20): **Devil Horn** (the synth horn) and **Star Horn** (the longer recovered production horn). `H` fires the **active** horn (hold-to-sound, C-9 unchanged); **`Ctrl+H`** toggles which horn is active (Devil ↔ Star) — a tap, not a hold. Toggling while a horn is sounding never cuts it; the switch applies to the next `H`. The horn button shows the active horn's name; the choice persists across restarts (library store) with a config default (`default_horn`, default **star**). Works armed (global hook) and focused. | Owner decision 2026-07-16 |
...
| C-20, C-46 | [app/assets/devil_horn_attack.wav](app/assets/devil_horn_attack.wav) · loop · tail, [app/assets/star_horn_attack.wav](app/assets/star_horn_attack.wav) · loop · tail (two three-part horns) |
| C-46 | [app/Audio/AudioEngine.cs](app/Audio/AudioEngine.cs) (two Horn machines + toggle), [app/Hotkeys/GlobalKeyboardHook.cs](app/Hotkeys/GlobalKeyboardHook.cs) (Ctrl+H), [app/MainWindow.xaml.cs](app/MainWindow.xaml.cs), [app/UI/SoundboardView.xaml](app/UI/SoundboardView.xaml) |
